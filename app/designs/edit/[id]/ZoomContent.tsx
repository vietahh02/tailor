...
import { getDesignById, updateDesignApi } from "@/app/util/api";
...
const ZoomContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = (await getDesignById(Number(id))) as any;
      if (res.message) {
        toast.info(
  ...
  const handleTryOnNewDesign = async () => {
    toast.info(
      "Hãy lưu bản thiết kế và quay lại trang list để có thể ướm lên tay bộ móng của mình!!!"
    );
  };
  ...
  const handleCapture = async () => {
    ...
    try {
      const dataUrl = await toPng(componentRef.current, {
        quality: 1,
        pixelRatio: 3, //window.devicePixelRatio
      });
  ...
  const handleSaveNewDesign = async () => {
    if (!name.trim()) {
      toast.error("Vui lòng nhập tên trước khi lưu thiết kế");
      return;
    }
